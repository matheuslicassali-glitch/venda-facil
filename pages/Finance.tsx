...
const Finance: React.FC<FinanceProps> = ({ onNotify, currentUser }) => {
    ...
    const handlePrint = () => {
        const rows = accounts.map(a => `
            <tr>
                ...
                <td><span class="badge ${a.tipo === 'pagar' ? 'badge-red' : 'badge-green'}">${a.tipo === 'pagar' ? 'DESPESA' : 'RECEITA'}</span></td>
                <td><span class="badge ${a.status === 'pago' ? 'badge-blue' : 'badge-orange'}">${a.status.toUpperCase()}</span></td>
                <td class="text-right">${a.tipo === 'pagar' ? '-' : '+'}${fmtCurPrint(a.valor)}</td>
            </tr>
        `).join('');

        const payRows = Object.entries(totals.byPayment).map(([k, v]) => `
            <tr><td>${k.replace('_', ' ').toUpperCase()}</td><td class="text-right">${fmtCurPrint(v as number)}</td></tr>
        `).join('');

        const body = `
            <div class="kpi-grid">
                <div class="kpi-card"><div class="kpi-label">Entradas Reais</div><div class="kpi-value">${fmtCurPrint(totals.receitaTotal)}</div></div>
                <div class="kpi-card"><div class="kpi-label">Despesas Pendentes</div><div class="kpi-value">${fmtCurPrint(totals.pendentePagar)}</div></div>
                <div class="kpi-card"><div class="kpi-label">Comissoes</div><div class="kpi-value">${fmtCurPrint(totals.comissoesTotal)}</div></div>
                <div class="kpi-card"><div class="kpi-label">Saldo Previsto</div><div class="kpi-value">${fmtCurPrint(totals.saldoPrevisto)}</div></div>
            </div>
            <div class="section-title">Recebimentos por Forma de Pagamento</div>
            <table><thead><tr><th>Meio</th><th class="text-right">Total</th></tr></thead><tbody>${payRows}</tbody></table>
            <div class="section-title">Lancamentos Financeiros</div>
            <table><thead><tr><th>Descricao</th><th>Categoria</th><th>Vencimento</th><th>Tipo</th><th>Status</th><th class="text-right">Valor</th></tr></thead><tbody>${rows}</tbody></table>
        `;
        printReport('Relatorio Financeiro', body);
    };
    ...
    const toggleStatus = async (account: FinancialAccount) => {
        ...
        try {
            await db.finance.save({ ...account, status: newStatus as any });
        ...
        <div className="animate-in fade-in duration-500 space-y-10 pb-10">
            <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-card text-card-foreground p-6 rounded-3xl border border-border shadow-sm mb-8">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                       ...
                       <h1 className="text-2xl font-black text-foreground tracking-tight">Gestão Financeira Pro</h1>
                    </div>
                    <p className="text-muted-foreground font-medium text-sm italic">Fluxo de caixa consolidado e controle de comissões</p>
                </div>
                <div className="flex gap-3">
                    <Button variant="ghost" onClick={handlePrint} className="border-border text-slate-600 h-12 px-6 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-background text-foreground">
                        🖨️ Relatório Financeiro
                    </Button>
                    {!isVendedor && (
                        <Button onClick={() => setIsModalOpen(true)} className="h-12 px-8 rounded-2xl bg-green-600 text-white font-black text-[10px] uppercase shadow-xl shadow-green-200 hover:bg-green-700 transition-all">
                            <Plus size={18} /> <span>Novo Título</span>
                        </Button>
                    )}
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    ...
                    { label: 'Saldo de Tesouraria', value: totals.saldoPrevisto, icon: Wallet, color: 'text-primary', bg: 'bg-blue-50' }
                ].map((item, idx) => (
                    <div key={idx} className="bg-card text-card-foreground p-6 rounded-3xl shadow-sm border border-border transition-all hover:shadow-md">
                        ...
                        <h4 className="text-2xl font-black text-foreground">{item.value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</h4>
                    </div>
                ))}
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="bg-card text-card-foreground rounded-3xl shadow-sm border border-border overflow-hidden lg:col-span-1">
                    <div className="p-5 border-b border-border bg-muted text-muted-foreground/50 flex justify-between items-center text-muted-foreground">
                    ...
                    <div className="p-6 space-y-5">
                        {[
                            ...
                            { label: 'Crédito Parcelado', key: 'cartao_credito', color: 'bg-primary text-primary-foreground' },
                ...
                <div className="bg-card text-card-foreground rounded-3xl shadow-sm border border-border overflow-hidden lg:col-span-2">
                    <div className="p-5 border-b border-border bg-muted text-muted-foreground/50 flex justify-between items-center text-muted-foreground">
                        <h3 className="font-black text-[10px] uppercase tracking-widest text-muted-foreground">Extrato de Movimentações Pendentes</h3>
                        <FileText size={16} />
                    </div>
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-100 font-medium">
                                {accounts.sort((a, b) => new Date(a.vencimento).getTime() - new Date(b.vencimento).getTime()).map(a => (
                                    <tr key={a.id} className="hover:bg-muted text-muted-foreground/80 transition-all group">
                                        <td className="px-6 py-4 flex items-center gap-3">
                                            <div className={`w-2 h-2 rounded-full ${a.tipo === 'pagar' ? 'bg-red-400' : 'bg-green-400'}`}></div>
                                            <div>
                                                <p className="font-black text-foreground text-sm leading-none mb-1 group-hover:text-primary transition-colors uppercase">{a.descricao}</p>
                                                <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">{a.categoria}</p>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="inline-flex items-center gap-2 px-2 py-1 bg-gray-100 rounded-lg text-xs font-black text-muted-foreground">
                                                <Calendar size={12} />
                                            ...
                                            <button 
                                                ...
                                                className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${
                                                    a.status === 'pago' 
                                                        ...
                                                        : 'bg-blue-100 text-primary hover:bg-blue-200 shadow-sm border border-blue-200'
                                ...
                                {accounts.length === 0 && (
                                    <tr>
                                        <td colSpan={4} className="py-24 text-center">
                                            ...
                                            <p className="text-muted-foreground font-black uppercase tracking-widest text-[10px]">Tesouraria equilibrada - nenhum pendente</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="💰 Novo Registro de Título Financeiro">
                <form onSubmit={handleSave} className="space-y-6 pt-4 custom-scrollbar">
                    <div className="p-4 bg-muted text-muted-foreground rounded-2xl border border-border space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Natureza do Título</label>
                                <select className="w-full px-4 py-2.5 bg-card text-card-foreground border border-border rounded-xl font-black text-gray-700 outline-none shadow-sm appearance-none"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Centro de Custo / Cat.</label>
                            <input 
                                className="w-full px-4 py-2.5 bg-muted text-muted-foreground border border-border rounded-xl font-bold text-gray-700 focus:outline-none focus:ring-2 focus:ring-ring appearance-none transition-all"
                    ...
                    <div className="flex justify-end gap-3 mt-10 pt-6 border-t sticky bottom-0 bg-card text-card-foreground">
